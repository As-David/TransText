...
class Trainer:
    ...
    def train(self):
        ...
        while True:
            ...
            np.savetxt("./training_losses.csv",self.training_losses,delimiter=',')
            epoch += 1
            if epoch > self.experiment.train.epochs:
                #if self.current_loss <= self.minloss:
    ...
    def train_step(self, model, optimizer, batch, epoch, step, **kwards):
        ...
        if step % self.experiment.logger.log_interval == 0:
            if isinstance(l, dict):
                ...
                print("-----------------------------> current_loss = %.6f" %(self.current_loss))
                print("-----------------------------> minloss      = %.6f" %(self.minloss))
                print("-----------------------------> minloss set in %d epochs" %(self.minloss_epoch))
                if self.current_loss <= self.minloss:
                    self.minloss = self.current_loss
                    self.minloss_epoch = epoch
                    print("-----------------------------> new minloss  = %.6f" %(self.minloss))
