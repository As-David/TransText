...
class L1BalanceCELoss(nn.Module):
    ...
    def forward(self, pred, batch):
        ...
        bce_loss_x_ref = self.bce_loss(pred['x_ref_loss'], batch['gt'], batch['mask'])
        bce_loss_x_4 = self.bce_loss(pred['x_4_loss'], batch['gt'], batch['mask'])
        bce_loss_x_5 = self.bce_loss(pred['x_5_loss'], batch['gt'], batch['mask'])
        ...
        if 'thresh' in pred:
            ...
            loss = dice_loss + self.l1_scale * l1_loss + bce_loss * self.bce_scale + bce_loss_x_ref + bce_loss_x_5 + bce_loss_x_4
            metrics.update(**l1_metric)
        else:
            loss = bce_loss
        return loss, metrics
